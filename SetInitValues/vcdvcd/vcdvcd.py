...
class VCDVCD(object):
    ...
    def get_endtime_value(self):
        return self.endtime_value
class Signal(object):
    """
...
class PrintDumpsStreamParserCallbacks(StreamParserCallbacks):
    ...
    def enddefinitions(self,vcd,signals,cur_sig_vals):
        ...
        # print(self._print_dumps_refs)
        # print(vcd.data)
        # print(cur_sig_vals)
        for i, ref in enumerate(self._print_dumps_refs, 1):
            # print('{} {}'.format(i, ref))
            # signal_name_list is a dict, add like {"self._references_to_widths[ref]": i}
            vcd.signal_name_list[ref] = i
            if i == 0:
                i = 1
    ...
    def time(self, vcd, time, cur_sig_vals):
        if (not self._deltas or vcd.signal_changed):
            ...
            for i, ref in enumerate(self._print_dumps_refs):
                ...
                # print(i, identifier_code, value)
                ss.append('{0:>{1}s}'.format(
                    binary_string_to_hex(value),
                    self._references_to_widths[ref])
                )
            # print(' '.join(ss))
            vcd.endtime_value.append(ss)
